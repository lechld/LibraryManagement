...
import at.aau.iteractivesystems.library.api.model.Subject
...
class RecommendationsRepositoryImpl(
    ...
    private fun mapItems(docs: List<Document>): List<Recommendation.Item> {
        return docs.mapNotNull { document ->
            val id = document.coverId ?: UUID.randomUUID().toString()
            val coverUrl = document.coverUrl ?: return@mapNotNull null
            val authorName = document.authorName.firstOrNull()
                ?: return@mapNotNull null // assume first author is nice enough
